...
const SMS = () => {
  ...
  const { data, refetch, fetchNextPage, isFetching, isFetchingNextPage } = useTanStackInfiniteQuery({
    ...
    getNextPageParam: lastPage => {
      if (searchOpen) {
        ...
        return lastPage.cursor.next_cursor || undefined;
      }
      return undefined;
    },
    ...
    placeholderData: searchOpen && keyword.length > 2 ? undefined : keepPreviousData,
    staleTime: 1000 * 60,
  });

  const displayedData = useMemo(() => {
    if (!data) return [];
    const _data = data.pages.reduce((acc, page) => [...acc, ...page.threads], []);
  ...
  const renderEmptyList = useCallback(() => {
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center' }}>
        <Text style={{ color: 'black', fontSize: 16, fontWeight: '400' }}>{message}</Text>
      </View>
    );
  }, [isFetching]);

  const renderFooter = useCallback(() => (
    <View style={styles.footer}>{isFetchingNextPage && <ActivityIndicator />}</View>
  ), [isFetchingNextPage]);
    ...
    displayedData.forEach((thread: ThreadResponse) => {
      ...
      TanStackQueryService.prefetchInfiniteQuery(
        ...
        async () => {
          ...
          return { sms: response?.sms.map(convertSMSToGiftedChatData) ?? [], cursor: response.cursor };
        },
        -1
      );
    });
  ...
  const renderItem = useCallback(({ item }: { item: ThreadResponse }) => (
  ...
  const keyExtractor = useCallback((item: ThreadResponse) => `sms-thread-${item.thread_id}`, []);

  useEffect(() => {
    const action = (viewID ?? '').split('-')[0] ?? '';
    if (action === 'idle') return;
    if (VIEW_ACTIONS.includes(action)) refetch();
  }, [refetch, viewID]);
